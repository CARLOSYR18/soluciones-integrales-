...
const electricalProjects = [
  ...
  },
  {
    ...
    description:
      "Instalación de alumbrado público en la obra Construcción de pistas, veredas. Centro Poblado Aldea Campesina Chancay.",
  },
...
const securityProjects = [
  { date: "SEPTIEMBRE DEL 2015", image: "https://i.postimg.cc/j2MVCJGx/Imagen4.png", description: "Descripción del proyecto de seguridad de septiembre 2015." },
  {
    ...
    description:
      "Montaje de torre para la conectividad de las cámaras de seguridad ciudadana. Municipalidad Distrital de San Ramon Chanchamayo.",
  },
...
const ProjectSlider: React.FC<ProjectSliderProps> = ({ title, projects }) => {
  ...
  return (
    <section className="py-16 px-6 max-w-7xl mx-auto mt-16">
      ...
      >
        <h2 className="text-3xl md:text-4xl font-black tracking-wide text-slate-900 uppercase">
          {title}
        </h2>
        <p className="mt-2 text-slate-500">Registro visual de trabajos realizados con enfoque profesional.</p>
      </motion.div>
      ...
      <motion.div
        ...
        transition={{ duration: 0.7, ease: "easeOut" }}
        viewport={{ once: true, amount: 0.25 }}
        className="relative rounded-3xl bg-white/70 backdrop-blur border border-slate-200 shadow-[0_30px_80px_rgba(0,0,0,0.12)] overflow-hidden"
      >
        <div className="pointer-events-none absolute -top-24 -right-24 h-72 w-72 rounded-full bg-cyan-300/25 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-24 -left-24 h-72 w-72 rounded-full bg-blue-300/25 blur-3xl" />

        <div className="relative p-5 md:p-7">
          <div className="relative rounded-2xl overflow-hidden border-2 border-cyan-400/70 shadow-[0_18px_50px_rgba(0,0,0,0.18)] bg-black">
            <div className="relative w-full h-[260px] sm:h-[380px] md:h-[520px] overflow-hidden">
              <motion.img
                key={activeIndex}
                src={current.image}
                alt={`Proyecto ${current.date}`}
              ...
              <div className="absolute bottom-0 left-0 w-full p-5 md:p-7 text-white">
                <motion.h3
                  key={current.date}
                ...
                <motion.p
                  ...
                  className="mt-2 text-sm md:text-base text-white/90 max-w-3xl leading-relaxed"
                >
                  {current.description}
                </motion.p>
              </div>
              ...
              <button
                onClick={handlePrev}
                className="group absolute left-4 top-1/2 -translate-y-1/2 z-10 h-12 w-12 rounded-full bg-white/10 border border-white/15 backdrop-blur flex items-center justify-center hover:bg-white/20 transition-all"
                aria-label="Anterior"
              >
              ...
              <button
                onClick={handleNext}
                className="group absolute right-4 top-1/2 -translate-y-1/2 z-10 h-12 w-12 rounded-full bg-white/10 border border-white/15 backdrop-blur flex items-center justify-center hover:bg-white/20 transition-all"
          ...
          <div className="mt-8 rounded-2xl border border-slate-200 bg-white/70 backdrop-blur p-4 md:p-5 shadow-sm">
            <div className="flex gap-3 overflow-x-auto pb-2 slider-tabs">
              {projects.map((project, idx) => {
                ...
                return (
                  <button
                    ...
                    className={[
                      "flex-shrink-0 rounded-xl px-4 py-3 text-left transition-all duration-300 border",
                      isActive
                        ? "bg-cyan-500 text-white border-cyan-400 shadow-[0_12px_30px_rgba(34,211,238,0.35)] scale-[1.02]"
                        : "bg-slate-50 text-slate-700 border-slate-200 hover:bg-cyan-50 hover:border-cyan-200",
                    ].join(" ")}
                  >
                    <div className="text-[11px] opacity-90 font-semibold tracking-wide">{project.date.split(" ")[0]}</div>
                    <div className="text-sm font-extrabold leading-tight">{project.date.split(" ").slice(1).join(" ")}</div>
                  </button>
                );
              })}
            </div>
              ...
              {projects.map((_, idx) => (
                <button
                  ...
                  className={[
                    ...
                    idx === activeIndex ? "w-8 bg-cyan-500" : "w-2.5 bg-slate-300 hover:bg-cyan-300",
                  ].join(" ")}
      ...
      <style>{`
        .slider-tabs::-webkit-scrollbar { height: 0px; }
        .slider-tabs { scrollbar-width: none; }
      `}</style>
    </section>
  );
};
            ...
            <div className="overflow-hidden" ref={carouselRef}>
              ...
              >
                {timelineItems.map((item, index) => (
                  ...
                  >
                    <div className="absolute -top-1 left-1/2 transform -translate-x-1/2 w-6 h-6 bg-cyan-400 rounded-full z-10 border-4 border-[#1a1a1a]" />
                    ...
                    {/* Para que el texto se vea “ordenado” como tu segunda cap */}
